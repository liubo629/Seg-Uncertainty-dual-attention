...
DATA_LIST_PATH = './dataset/ACDC_list/train.txt'
SAVE_PATH = './data/ACDC/data/pseudov3+danet/train'

if not os.path.isdir('./data/ACDC/data/pseudov3+danet/'):
    os.mkdir('./data/ACDC/data/pseudov3+danet/')
    os.mkdir(SAVE_PATH)
    ...
    for index, img_data in enumerate(zip(testloader, testloader2) ):
        ...
        if args.model == 'DeepLab':
            with torch.no_grad():
                ...
                sink ,_ ,_ = sinkhorn(output1, output2 )
        ...
        for i in range(output_batch.shape[0]):
            ...
            uncertainty = np.sum(heatmap_batch[i,:,:])*0.001
            fig = plt.figure()
            plt.axis('off')
            heatmap = plt.imshow(heatmap_tmp, cmap='viridis')
            fig.colorbar(heatmap)
            plt.title('uncertainty = %f sink = %f' %(uncertainty,sink[i]))
