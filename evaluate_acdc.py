...
DATA_DIRECTORY = './data/acdc'
DATA_LIST_PATH = './dataset/ACDCs_list/val.txt'
SAVE_PATH = './result/acdc2'

IGNORE_LABEL = 255
NUM_CLASSES = 19
NUM_STEPS = 500 # Number of images in the validation set.
RESTORE_FROM = './GTA5_25000.pth'
...
def main():
    ...
    testloader = data.DataLoader(acdcDataSet(args.data_dir, args.data_list, crop_size=(512,1024), resize_size=(1024, 512), mean=IMG_MEAN, scale=False, mirror=False, set=args.set),
                                    batch_size=batchsize, shuffle=False, pin_memory=True, num_workers=4)

    scale = 1.25
    testloader2 = data.DataLoader(acdcDataSet(args.data_dir, args.data_list, crop_size=(round(512*scale), round(1024*scale) ), resize_size=( round(1024*scale), round(512*scale)), mean=IMG_MEAN, scale=False, mirror=False, set=args.set),
                                    batch_size=batchsize, shuffle=False, pin_memory=True, num_workers=4)
    scale = 0.9
    testloader3 = data.DataLoader(acdcDataSet(args.data_dir, args.data_list, crop_size=(round(512*scale), round(1024*scale) ), resize_size=( round(1024*scale), round(512*scale)), mean=IMG_MEAN, scale=False, mirror=False, set=args.set),
                                    batch_size=batchsize, shuffle=False, pin_memory=True, num_workers=4)
    ...
    for index, img_data in enumerate(zip(testloader, testloader2, testloader3) ):
        ...
        if args.model == 'DeepLab':
            with torch.no_grad():
                ...
                #sink ,_ ,_ = sinkhorn(heatmap_output1, heatmap_output2 )
                #heatmap_batch = torch.sum(sink, dim =1) 
                #a = log_sm(heatmap_output1)
                #b = sm(heatmap_output2)
                #c = kl_distance(a,b)
