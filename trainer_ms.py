    ...
    def gen_update(self, images, images_t, labels, labels_t, i_iter):
            ...
            if self.lambda_kl_target_copy>0:
                ...
                with torch.no_grad():
                    ...
                    mean_pred2 = self.sm(0.5*pred1 + pred2)#计算源域
                loss_kl = ( self.kl_loss(self.log_sm(pred_target2) , mean_pred)  + self.kl_loss(self.log_sm(pred_target1) , mean_pred))/(n*h*w)
                loss_kl2 = ( self.kl_loss(self.log_sm(pred2) , mean_pred2)  + self.kl_loss(self.log_sm(pred1) , mean_pred2))/(n2*h2*w2)#试着作为smoothl1loss的一个值
                ...
                loss_sml1 = self.smoothl1_loss(loss_kl, loss_kl2)
                print(loss_kl)
                loss += self.lambda_kl_target * loss_kl + self.lambda_kl_target * loss_sml1
